...
const AnalysisDashboard = () => {
    ...
    const fullCriteriaList = [
        ...
        {
"id": 17,
"displayText": "Verificar a quantidade de imagens, infográficos, tabelas e mapas mentais.",
"type": "auto",
"textForAI": `Este critério deve ser SEMPRE 'Aprovado'. Sua tarefa é contar o número de mapas mentais (buscando menções textuais ao título ou seção 'Mapa Mental' ou similar), infográficos (buscando menções como 'infográfico', 'DG - Refazer o infográfico' ou descrições de infográficos), tabelas (buscando menções como 'tabela' ou estruturas de tabela no texto) e imagens (buscando menções como 'imagem', 'ilustração', 'Stock Imagem vetorial ID' ou descrições de cenas para ilustradores no texto ou comentários). Considere a ementa: ${selectedEmenta ? selectedEmenta.conteudo_programatico : '[CONTEUDO_PROGRAMATICO]'}. Na justificativa, informe as quantidades encontradas no formato: 'Foram encontrados X mapas mentais, Y infográficos, Z tabelas e W imagens no total.`
},
        {
            "id": 18,
    ...
    const handleEditClick = (criterion) => {
        // Sempre abre o modal ao clicar no lápis
        setCorrectionTarget(criterion);
        setIsCorrectionModalOpen(true);

        // A geração de correção via IA só deve acontecer para critérios reprovados
        if (criterion.status !== 'Reprovado') {
            setSuggestion(null);
            return;
        }

        // E apenas para critérios configurados para receber sugestão da IA
            ...
            try {
                if (token) {
                    ...
                    await fetch(`${apiBase}/api/relatorios`, {
                        ...
                        body: JSON.stringify({
                            titulo: `Relatório - Aluno - ${studentFile.name}`,
                            tipo: 'aluno',
                            conteudo: {
                                aba: 'aluno',
                                arquivo: studentFile.name,
                ...
                console.error('Erro ao salvar relatório (aluno):', err);
            }
    ...
    return (
        <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
                ...
                {isLoadingEmentas ? (
                    ...
                    <Autocomplete
                        ...
                        <TextField
                            ...
                            helperText="Escolha a ementa para contextualizar a análise."
                        />
                        )}
                    />
            ...
            {/* Indicador de carregamento (análise de links + análise geral) */}
            {(isAnalyzingLinks || isLoading) && (
            <Box sx={{ width: '100%', mt: 4 }}>
                <Typography textAlign="center" sx={{ mb: 2 }}>
                {isAnalyzingLinks
                    ? linkAnalysisMessage
                    : 'Analisando documento completo...'}
                </Typography>
                {isAnalyzingLinks ? (
            ...
            <Paper
                elevation={3}
                sx={{ mt: 4, p: { xs: 2, md: 3 }, textAlign: 'center' }}
            >
                <Typography variant="h5" gutterBottom>
                Análise Concluída
                </Typography>
                <Typography variant="h6">
                Pontuação da Análise Automática: {analysisResult.pontuacaoFinal}% de
                Aprovação
                </Typography>
                <Box
                sx={{
