    ...
    return (
        ...
        >
            ...
            >
                ...
                sx={{ flexGrow: 1, maxWidth: { xs: '100%', sm: 220 } }}
                >
                {showAll
                    ? `Apenas Reprovados (${reprovados.length})`
                    : `Ver Outros Critérios (${otherCriteria.length})`}
                </Button>
            ...
            <Table stickyHeader size="small">
                ...
                {finalItems.length > 0 ? (
                    finalItems.map((item) => {
                    const isEmentaRow =
                        item.descricao === 'Validar conformidade com a ementa';

                    // 1. Linha especial para o critério principal (ementa)
                    if (isEmentaRow) {
                        return (
                        <TableRow key={item.criterio}>
                            ...
                            >
                                <strong>Validações realizadas:</strong> Este critério
                                verifica se a apostila está completamente alinhada com a
                                ementa selecionada, considerando todos os seus elementos
                                (objetivos e conteúdo programático). A análise responde
                                a duas perguntas fundamentais:
                                <Box component="ul" sx={{ pl: 2, mt: 1 }}>
                                <li>
                                    <strong>
                                    "Se eu ler toda a apostila, vou aprender a fazer o
                                    que o objetivo da ementa espera?"
                                    </strong>{' '}
                                    Verifica-se se a apostila contém explicações,
                                    exemplos, exercícios e atividades práticas
                                    suficientes para alcançar cada objetivo da ementa,
                                    com clareza, profundidade e progressão adequadas.
                                </li>
                                <li>
                                    <strong>
                                    "Todo o conteúdo programático está sendo
                                    explorado?"
                                    </strong>{' '}
                                    Confirma-se se todos os itens do conteúdo
                                    programático estão presentes na apostila, com os
                                    mesmos nomes (ou sinônimos claros) e no contexto
                                    correto, incluindo referências locais quando
                                    aplicável.
                                </li>
                                </Box>
                                ...
                                <Box component="ul" sx={{ pl: 2, mt: 1 }}>
                                <li>
                                    <strong>Objetivos atendidos:</strong>{' '}
                                    {item.status === 'Aprovado'
                                    ? 'Sim, todos os objetivos da ementa foram completamente atendidos com conteúdo suficiente.'
                                    : 'Não, pelo menos um objetivo não foi atendido adequadamente. Veja a justificativa para detalhes.'}
                                </li>
                                <li>
                                    <strong>Conteúdo programático explorado:</strong>{' '}
                                    {item.status === 'Aprovado'
                                    ? 'Sim, todos os itens do conteúdo programático estão presentes e no contexto correto.'
                                    : 'Não, pelo menos um item do conteúdo programático está ausente ou desalinhado. Veja a justificativa para detalhes.'}
                                </li>
                                </Box>
                            </Box>
                            </TableCell>
                    ...
                    // 2. Linhas padrão (demais critérios)
                    const hasSuggestion =
                        onEditCriterion &&
                        criteriaWithSuggestions.includes(item.criterio);

                    return (
                        <TableRow key={item.criterio} hover>
                        ...
                        {item.status === 'Reprovado' && onEditCriterion && (
                            ...
                            title={
                                hasSuggestion
                                ? 'Ver sugestão de correção (IA)'
                                : 'Marcar como corrigido'
                            }
                            >
                            <IconButton
                                size="small"
                                onClick={() => onEditCriterion(item)}
                            >
                                <EditIcon
                                color={hasSuggestion ? 'primary' : 'action'}
                                fontSize="small"
                                />
                            </IconButton>
                            </Tooltip>
                    ...
                    <TableCell colSpan={4} align="center">
                        ...
                        {showAll
                            ? 'Nenhum outro critério disponível para exibição.'
                            : 'Nenhum outro critério reprovado encontrado.'}
                        </Typography>
                    </TableCell>
                    </TableRow>
                )}
